...
def sample_images():
    ...
    save_path = os.path.join(config.MODEL_DIR, 'models', 'my_model_colorization.h5')
    print(save_path)
    ...
    for b in range(total_batch):
            ...
            for i in range(config.BATCH_SIZE):
                ...
                save_path = os.path.join(config.OUT_DIR, 'test_%d_%d' %(b, i), "{:4.8f}_".format(psnr)+filelist[i][:-4] +"psnr_reconstructed.jpg" )
