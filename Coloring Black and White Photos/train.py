...
def reconstruct(batchX, DIR, predictedY, filelist):

    result = np.concatenate((batchX, predictedY), axis=2)
    result = cv2.cvtColor(result, cv2.COLOR_Lab2BGR)
    # save_results_path = os.path.join(config.OUT_DIR,config.TEST_NAME)
    save_results_path = config.OUT_DIR +DIR
...
class MODEL():
    ...
    def train(self, train_data,test_data,sample_interval=1):

        # Create folder to save models if needed.
        save_models_path =os.path.join(config.MODEL_DIR, 'models')
        # print(save_models_path)
        ...
        total_batch = int(train_data.size/config.BATCH_SIZE)
        for epoch in range(config.NUM_EPOCHS):
            for batch in range(total_batch):
                # print('batch', batch)
                # new batch
                trainL, trainAB, _, original, l_img_oritList  = train_data.generate_batch()
                ...
                if batch%2==0:
                    save_path = os.path.join(save_models_path, "my_model_combined_%d_%d.h5" % (epoch, batch))
                    self.combined.save(save_path)
                    save_path = os.path.join(save_models_path, "my_model_colorization_%d_%d.h5" % (epoch, batch))
                    self.colorizationModel.save(save_path)
                    save_path = os.path.join(save_models_path, "my_model_discriminator_%d_%d.h5" % (epoch, batch))
                    self.discriminator.save(save_path)

                    # sample images after each epoch
                    self.sample_images(test_data,epoch, batch)


    def sample_images(self,test_data,epoch, batch):
        ...
        for _ in range(total_batch):
            ...
            for i in range(config.BATCH_SIZE):
                ...
                predResult = reconstruct(deprocess(labimg_ori), '/epoch%dbatch%d' %(epoch, batch), predictedAB, "epoch"+str(epoch)+"_"+filelist[i][:-5] )
            break
...
if __name__ == '__main__':
    ...
    colorizationModel.train(train_data,test_data)
